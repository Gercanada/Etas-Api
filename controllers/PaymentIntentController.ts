...
export const convertReq = (req: Request, res: Response) => {
    try {
        // json_encode( array('item_type_id' => "item_type",     'string_key' => 'string_key',     'string_value' => 'string_value',     'string_extra' => 'string_extra',     'is_public' => 'public',     'is_public_for_contacts' => 'meh'
        // res.status(200).json(JSON.stringify(req?.body))
        console.log({req:(req?.body)});
        // console.log({parsed:JSON.parse(req?.body?.body)});
        // console.log({stringify:JSON.stringify(req?.body?.body)});
